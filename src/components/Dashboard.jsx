...
export default function Dashboard({ onViewMovie }) {
  ...
  const [filterType, setFilterType] = useState('all'); // Nuevo estado para filtrar por tipo
  ...
  const fetchMediaWithData = async () => {
    ...
    if (itemsFromDb) {
      const itemsWithData = await Promise.all(itemsFromDb.map(async (m) => {
        ...
        try {
          // Detectamos si es tv o movie para llamar al endpoint correcto
    ...
    <div>
      <div className="mb-6 flex gap-4 items-center flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <span className="font-bold text-gray-700">Mostrar:</span>
        ...
        {filteredAndSortedItems.map(m => (
          <div key={m.id} className="border p-4 rounded shadow bg-white cursor-pointer hover:shadow-lg transition" onClick={() => onViewMovie(m.id)}>
            ...
            <div className="mt-2">
              ...
              <h2 className="font-bold text-lg">{m.title} ({m.year > 0 ? m.year : 'N/A'})</h2>
            </div>

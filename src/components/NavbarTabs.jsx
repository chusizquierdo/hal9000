    ...
    <div className="max-w-7xl mx-auto px-4 mt-4 relative">
      ...
      <div className="md:hidden relative w-full z-30">
        ...
        >
          ...
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={3} stroke="currentColor" className={`w-4 h-4 text-gray-900 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}>
            ...
            <div className="absolute left-0 right-0 mt-2 bg-white border-2 border-gray-900 rounded-xl shadow-xl z-40 py-1.5 overflow-y-auto max-h-96">
              {mainTabs.map((tab) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-2.5 text-sm font-bold transition-colors flex items-center justify-between ${activeTab === tab.id ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-gray-50'}`}
                >
                  <span>{tab.label}</span>
              ...
              <div className="border-t border-b border-gray-100 my-1">
                <button
                  ...
                  className={`w-full text-left px-4 py-2.5 text-sm font-bold transition-colors flex items-center justify-between ${isGameActive ? 'text-blue-600 bg-blue-50/40' : 'text-gray-600 hover:bg-gray-50'}`}
                >
                  ...
                  <div className="bg-gray-50/90 pl-4">
                    {gameTabs.map((subTab) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-2 text-sm font-bold transition-colors flex items-center justify-between ${activeTab === subTab.id ? 'text-blue-600 font-black' : 'text-gray-500 hover:text-gray-800'}`}
                      >
              ...
              <button
                ...
                className={`w-full text-left px-4 py-2.5 text-sm font-bold transition-colors flex items-center justify-between ${activeTab === 'contact' ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-gray-50'}`}
              >
      ...
      <div className="hidden md:flex flex-wrap gap-x-6 gap-y-2 border-b border-gray-200 pb-2 items-center">
        {mainTabs.map((tab) => (
          <button 
            ...
            className={`font-bold whitespace-nowrap transition-colors border-b-2 ${
        ...
        >
          <button
            ...
            className={`font-bold whitespace-nowrap transition-colors flex items-center gap-1 border-b-2 outline-none ${
